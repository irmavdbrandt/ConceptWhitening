...
class DeepMirResNetTransferv3(nn.Module):
    def __init__(self, args, whitened_layers=None, model_file=None):
        super(DeepMirResNetTransferv3, self).__init__()
        self.model = DeepMirResNetv3()
        print(model_file)
        if model_file is not None:
            if not os.path.exists(model_file):
                raise Exception("checkpoint {} not found!".format(model_file))
...
class DeepMirResNetv3(nn.Module):

    def __init__(self) -> None:
        super(DeepMirResNetv3, self).__init__()
        self.conv1 = nn.Conv2d(3, 48, kernel_size=3, stride=(1, 1))
        self.relu1 = nn.ReLU()
        self.conv2 = conv3x3(48, 48)
        self.relu2 = nn.ReLU()
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        out = self.pool4(out)
        out = torch.mean(out, dim=3)  # used to be dim=(2, 3), now I do the flatten afterwards
...
class DeepMirResNetBNv3(nn.Module):
    def __init__(self, args, model_file=None):
        super(DeepMirResNetBNv3, self).__init__()
        self.model = DeepMirResNetv3()
        if model_file is not None:
            checkpoint = torch.load(model_file, map_location='cpu')
            args.start_epoch = checkpoint['epoch']
